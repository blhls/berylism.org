...
(function () {
  ...
  var FRAG = [
    ...
    '  vec3 c = cAby;',
    '  c = mix(c, cTyr, smoothstep(0.02,0.24,lit));',
    '  c = mix(c, cInd, smoothstep(0.18,0.50,lit));',
    '  c = mix(c, cPet, smoothstep(0.46,0.78,lit));',
    '  c = mix(c, cAqu, smoothstep(0.86,1.00,lit)*0.55);',

    '  float rim = pow(1.0 - clamp(dot(vN,vec3(0.0,0.0,1.0)),0.0,1.0), 3.0);',
    '  c += cPet * rim * 0.14 * vF;',

    '  vec3 flatc = mix(cAby, cTyr, 0.62 + noise(vUv*38.0)*0.10);',
  ...
  var Zarqa = {
    ...
    init: function () {
      ...
      this.uni = {
        ...
        cAby: { value: C(0x0B0416) }, cTyr: { value: C(0x35093F) },
        cInd: { value: C(0x1E2E6E) }, cPet: { value: C(0x00718C) },
  ...
  var Consoles = {
    ...
    build: function () {
      ...
      this.notice = shell('c-notice', 'BOP',
        '<h1 class="mtl-title"><span class="g">✫</span> ･ ﾟ ✴ &nbsp;N O T I C E&nbsp; ✴ ﾟ ･ <span class="g">✫</span></h1>' +
    ...
    openHelp: function () {
      ...
      var el = shell('c-help', 'ASK FOR HELP',
        '<p class="mtl-lead">state your situation. it will be carried.</p>' +
    ...
    ack: function () {
      ...
      var el = shell('c-ack', 'TRANSMISSION',
        ...
        '<p class="mtl-lead dim">✫ ･ ﾟ ✴ ꨄ ✧ ･ ﾟ ✦ ✩ ♡ ⋆ ✿ ☆</p>', true);
      this.host.insertBefore(el, this.notice);
      requestAnimationFrame(function () { el.classList.add('in', 'glitching'); });
      setTimeout(function () { el.classList.remove('glitching'); }, 430);
    },
